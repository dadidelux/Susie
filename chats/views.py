...
def home(request):
    ...
    if latest_chat is None:
        new_chat_title = create_new_chat_session(request)
        chat_history = ChatHistory.objects.filter(chat_session__user=request.user, chat_session__title=new_chat_title).order_by('timestamp')
    else:
        request.session['session_title'] = latest_chat.title
        chat_history = ChatHistory.objects.filter(chat_session__user=request.user, chat_session__title=latest_chat.title).order_by('timestamp')
